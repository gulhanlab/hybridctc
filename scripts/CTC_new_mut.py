...
exclusive_ctc_df_homozygous_filtered_snvs = pd.read_csv('/n/data1/hms/dbmi/gulhan/lab/ankit/scripts/mutation_calling/HybridCTC/exclusive_ctc_df_homozygous_filtered_snvs.csv')
...
for bam_file in os.listdir(bam_folder):
    if bam_file.endswith('.rmdup.sorted.bam'):  # Filter for BAM files
        output_file = os.path.join(output_folder, f"CTC_alt_read_support_{os.path.splitext(bam_file)[0]}.csv")

        # Check if output file already exists
        if os.path.exists(output_file):
            print(f"Skipping {bam_file} as output already exists: {output_file}")
        ...
        for idx, row in exclusive_ctc_df_homozygous_filtered_snvs.iterrows():
